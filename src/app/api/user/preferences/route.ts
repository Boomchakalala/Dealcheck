...
export async function GET() {
  try {
    ...
    const { data: profile } = await supabase
      .from('profiles')
      .select('negotiation_preferences')
      .eq('id', user.id)
      .single()

    return NextResponse.json({ preferences: profile?.negotiation_preferences || null })
  } catch {
...
export async function POST(request: Request) {
  try {
    ...
    const { error } = await supabase
      .from('profiles')
      .update({ negotiation_preferences: preferences } as any)
      .eq('id', user.id)
