...
function CategoryTag({ category }: { category: string }) {
  return (
    <span
      className="inline-flex items-center text-[10.5px] font-bold uppercase tracking-wider text-emerald-700 bg-emerald-50 px-2.5 py-1 rounded-md whitespace-nowrap flex-shrink-0"
    ...
    <>
      ...
      {featured && (
        <Link href={`/blog/${featured.slug}`} className="block group no-underline mb-6">
          <article
            className="rounded-3xl p-7 sm:p-10 border-2 border-emerald-200 transition-all hover:-translate-y-1 hover:border-emerald-400 hover:shadow-xl"
          ...
          >
            ...
            <h2
              className="font-bold text-slate-900 mb-3 group-hover:text-emerald-700 transition-colors"
              style={{ fontFamily: sora, fontSize: 'clamp(26px, 3.4vw, 40px)', lineHeight: 1.12, letterSpacing: '-0.025em' }}
            >
              {featured.title}
            </h2>
            <p className="text-[15.5px] sm:text-[16.5px] text-slate-500 leading-relaxed max-w-2xl">{featured.description}</p>
          </article>
        </Link>
      )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {rest.map((post) => (
            <Link key={post.slug} href={`/blog/${post.slug}`} className="block group no-underline">
              <article className="h-full flex flex-col rounded-2xl p-6 bg-white border border-slate-200 transition-all hover:-translate-y-1 hover:border-emerald-400 hover:shadow-lg">
                <div className="mb-3">
                  <CategoryTag category={post.category} />
                </div>
                <h3
                  className="font-bold text-slate-900 mb-2 group-hover:text-emerald-700 transition-colors"
                  style={{ fontFamily: sora, fontSize: 19, lineHeight: 1.25, letterSpacing: '-0.02em' }}
                >
                  {post.title}
                </h3>
                <p className="text-[14px] text-slate-500 leading-relaxed mb-5 line-clamp-2">{post.description}</p>
      ...
      {filtered.length === 0 && (
        <p className="text-center text-[15px] text-slate-400 py-12">
