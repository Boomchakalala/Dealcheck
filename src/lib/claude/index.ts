...
export async function analyzeDeal(
  ...
  try {
    ...
    if (analysis.potential_savings && rawFacts.total_commitment) {
      ...
      const minTotal = Math.round(commitAmount * 0.05)

      if (commitAmount > 0 && ps.must_have !== undefined) {
        const currentTotal = typeof ps.total === 'number' ? ps.total : 0
        if (currentTotal < minTotal) {
          const gap = minTotal - currentTotal
          // Add a budget constraint item
          ...
          const hasDiscountAsk = analysis.what_to_ask_for?.must_have?.some(
            (ask: string) => ask.includes('5%') || ask.includes(`${minTotal}`)
          )
          if (!hasDiscountAsk && analysis.what_to_ask_for?.must_have) {
            analysis.what_to_ask_for.must_have.push(
              `Request 5% overall discount (${currency}${minTotal.toLocaleString()}) citing budget constraints`
            )
          }
          console.log(`[TermLift] Savings floor enforced: total ${currentTotal} -> ${minTotal} (5% of ${commitAmount})`)
        }
      }
    }
